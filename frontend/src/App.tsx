...
import "./App.css";
import { PetitionDapp } from "./contracts";
...
const CONTRACT_ID = "0x80221a7efa73d1da9d8c29404139903f91be5a581488e32f2327ce872d80f170"; //Replace with your contract address
...
function App() {
  ...
  async function getDeadline(deadlineDays: number) {
    ...
    const deadline =
      currentTimestamp.toUnixSeconds() + oneDayInSeconds * deadlineDays;
  ...
  async function createCampaign(deadline: number) {
    if (isConnected && wallet) {
      const contract = new PetitionDapp(CONTRACT_ID, wallet);
  ...
  async function campaignInfo(campaignId: number) {
    if (isConnected && wallet) {
      ...
      const deadlineTimestamp =  convertBNToDate(value?.deadline);
